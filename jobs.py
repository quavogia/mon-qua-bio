    ...
    async def daily_topic_suggest(self, ctx):
        ...
        for key, ch in self.config.channels.items():
            ...
            msg = (
                ...
                f"📅 {date_str}\n\n"
                f"{text}\n\n"
                "👉 Thích chủ đề nào, nhắn: chốt: <tên chủ đề> — Hermes sẽ viết script + tạo "
                "bản nháp để hệ tự dựng. (Hermes chỉ GỢI Ý, không tự chốt/đăng.)"
            )
            await self._send(ctx, msg, channel=key)
