...
class Scheduler:
    ...
    async def daily_topic_suggest(self, ctx):
        ...
        for key, ch in self.config.channels.items():
            ...
            try:
                _plog = self.project_log.read() if self.project_log else ""
            except Exception:  # noqa: BLE001
                _plog = ""  # fail-open: không có sổ vẫn gợi ý bình thường
            system = build_system(ch, [], _plog) + (
                ...
                "ngắn>'.\n"
                "- ƯU TIÊN THAM CHIẾU: nếu trong SỔ NHẬT KÝ DỰ ÁN có ghi chú nguồn "
                "'bao-cao-tuan' (CỐ VẤN TĂNG TRƯỞNG + ĐỀ XUẤT TUẦN TỚI / 7 chủ đề) gần nhất "
                "ĐÚNG kênh này, hãy ưu tiên kế thừa/biến tấu từ các chủ đề đề xuất đó.\n"
